...
var redraw = function() {
  ...
  r.risk_prelude.attr({
    text:
      "If you're " +
      (p.smoker_p.value ? '' : 'not ') +
      'a smoker with a blood pressure\n of ' +
      p.sbp.value +
      'mm/Hg ' +
      (p.fx_of_mi_p.value ? 'and have ' : "and don't have ") +
...
var draw_visualization = function() {
  ...
  var c = null;
  r.g
    .text(500, 55 - 24, 'Note: these results are valid for non-diabetics only!')
  ...
  r
    ...
    .hover(
      function() {
        this.attr({ fill: '#F4804F', cursor: 'pointer' });
      },
      function() {
        if (!p.smoker_p.value) this.attr({ fill: '#F6F6F6', cursor: 'normal' });
      }
    )
    .click(function() {
  ...
  r
    ...
    .hover(
      ...
      function() {
        if (!p.fx_of_mi_p.value) this.attr({ fill: '#F6F6F6', cursor: 'normal' });
      }
    )
    .click(function() {
  ...
  r.g
    .text(500 + 20 + 10, 55 + 24, 'Family history of heart attack?')
    .attr({ 'font-size': '16px', 'font-weight': 'normal' });

  // SBP slider
  r.g
    .text(500 + 20 + 10, 55 + 24 + 24, 'Systolic blood pressure')
    .attr({ 'font-size': '16px', 'font-weight': 'normal' });
  var min_x = 415,
    max_x = 515,
    len_x = max_x - min_x,
  ...
  r
    .path('M' + min_x + ' ' + y + ' L' + max_x + ' ' + y)
    .attr({ stroke: '#aaa', 'stroke-dasharray': '.', 'stroke-linecap': 'butt' });
  var t = r.g.text(start_x, y, p.sbp.value).attr({
    cursor: 'pointer',
  ...
  r.g
    .text(30, y, '1')
    .attr({ font: '18px Consolas, monospace', 'text-anchor': 'middle', 'font-weight': 'bold', fill: '#fff' });
  r.g.text(50, y, 'About this test').attr({ 'font-size': '18px', 'font-weight': 'bold' });
  r.g
    .text(50, y + 20, 'This report evaluates your potential risk of heart disease, heart attack, and stroke.')
    .attr({ fill: '#888' })
    .attr({ 'font-size': '14px' });
  ...
  r.g
    .text(x + unit_w * 3, y + h + 8, 'High risk of cardiovascular disease')
    .attr({ 'font-size': '12px', 'font-weight': 'normal' });
  r.g.text(x + unit_w * 3, y + h + 20, '3 - 10').attr({ 'font-size': '10px', 'font-weight': '200', fill: '#888' });

  r
    .path('M' + cap_x + ' ' + cap_y + 'L' + cap_mid_x + ' ' + cap_mid_y + 'L' + cap_end_x + ' ' + cap_end_y + 'z')
  ...
  r.g
    .text(circle_x, y, p.hsCRP.value.toPrecision(2))
    .attr({ font: '20px Consolas, monospace', 'text-anchor': 'middle', 'font-weight': 'normal', fill: '#fff' });
  ...
  r.circle(circle_x, y, 22).attr({ fill: '#F4804E', stroke: '#fff', 'stroke-width': '2px' });
  r.g
    .text(circle_x, y, parseInt(p.cholesterol.value))
    .attr({ font: '20px Consolas, monospace', 'text-anchor': 'middle', 'font-weight': 'normal', fill: '#fff' });
  ...
  r.g
    .text(circle_x, y, parseInt(p.LDL.value))
    .attr({ font: '16px Consolas, monospace', 'text-anchor': 'middle', 'font-weight': 'normal', fill: '#fff' });
  ...
  var circle_x = unit_w * (p.HDL.value / 18) + x;
  r.circle(circle_x, y, 18).attr({ fill: '#F4804E', stroke: '#fff', 'stroke-width': '2px' });
  r.g
    .text(circle_x, y, parseInt(p.HDL.value))
    .attr({ font: '16px Consolas, monospace', 'text-anchor': 'middle', 'font-weight': 'normal', fill: '#fff' });
  ...
  r.g
    .text(50 + 100, 720, 'You show an elevated risk of cardiovascular disease')
  ...
  r.score_text = r.g.text(320, 780, score + '%').attr({
    'font-size': '72px',
    'font-weight': 'bold',
    fill: '#6A9C2D',
  ...
  r.score_if_sbp_of_10_lower_text = r.g
    .text(460, y + 16, score_if_sbp_of_10_lower + '%')
    .attr({ 'font-size': '14px', 'font-weight': 'bold', fill: '#555' });
  r.sbp_10_lower = r.g
    .text(450 + 36, y + 16, 'if your blood pressure were ' + sbp_10_lower + 'mm/Hg')
    .attr({ 'font-size': '14px', fill: '#888' });

  r.score_if_all_optimal_text = r.g
    .text(460, y + 32, score_if_all_optimal + '%')
    .attr({ 'font-size': '14px', 'font-weight': 'bold', fill: '#555' });
  r.g
    .text(450 + 36, y + 32, "if you didn't smoke and all levels were optimal")
    .attr({ 'font-size': '14px', fill: '#888' });
  ...
  r.g
    .text(50, y + 80, 'Use your test results to calculate your risk of a cardiovascular event at ReynoldsRisk.org')
    .attr({ 'font-size': '14px', 'font-weight': 'normal', fill: '#888' });
  ...
  r.quit_text_1 = r.g
    .text(x + 180 + 55, y + 12, 'Staying smoke-free')
    .attr({ 'font-size': '14px', 'font-weight': 'bold', fill: '#555' });
  r.quit_text_2 = r.g
    .text(x + 180 + 55, y + 36 + 8, 'is one of the best\nways to improve your\nheart disease risk')
    .attr({ 'font-size': '14px', fill: '#888' });
  ...
  r.g
    .text(
      90,
      1000,
      'Original Design: David McCandless & Stefanie Posavec for Wired Magazine // informationisbeautiful.net'
    )
    .attr({ 'font-size': '14px', fill: '#0088CC' })
    .click(function(e) {
      window.open('http://www.informationisbeautiful.net/2010/visualizing-bloodtests/');
    })
    .hover(
      function(e) {
  ...
  r.g
    .text(90, 1000 + 16, 'Reynolds Risk Score Calculator // ReynoldsRiskScore.org')
    .attr({ 'font-size': '14px', fill: '#0088CC' })
    .click(function(e) {
      window.open('http://ReynoldsRiskScore.org');
    })
    .hover(
      function(e) {
        this.attr('cursor', 'pointer');
  ...
  r.g
    ...
    .click(function(e) {
      window.open('http://jama.ama-assn.org/content/297/6/611.long');
    })
    .hover(
      function(e) {
        this.attr('cursor', 'pointer');
  ...
  r.g
    ...
    .attr({ 'font-size': '12px', fill: '#0088CC' })
    .click(function(e) {
      window.open('http://circ.ahajournals.org/cgi/content/full/118/22/2243');
    })
    .hover(
      function(e) {
        this.attr('cursor', 'pointer');
